...
export const AssessmentPreviewModal: React.FC<AssessmentPreviewModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-xs p-4 animate-in fade-in">
      <div className="bg-white rounded-3xl border border-[#e5e5e5] shadow-xl max-w-2xl w-full max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95">
        {/* Header */}
        <div className="px-6 py-4 border-b border-[#e5e5e5] flex items-center justify-between bg-white text-[#222222]">
          <div className="flex items-center gap-2.5">
            <span className="px-3 py-1 text-xs font-bold uppercase rounded-full bg-[#fef3e2] text-[#b45309] border border-[#f8af43]/30">
              Preview Mode
            </span>
            <span className="text-sm font-extrabold text-[#222222] truncate max-w-xs">{courseName}</span>
          </div>
        ...
        {questions.length === 0 ? (
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            {/* Question Counter */}
            <div className="flex items-center justify-between text-xs text-[#616161] pb-2 border-b border-[#e5e5e5]">
            ...
            <div
              dir="auto"
              className={`text-lg font-bold text-[#222222] leading-relaxed ${
                ...
                {currentQuestion.choices.map((c, i) => {
                  ...
                  return (
                    <div
                      key={c.id}
                      dir="auto"
                      className={`p-3.5 rounded-2xl border border-[#e5e5e5] bg-[#fafafa] hover:bg-[#e6eff8]/40 transition-colors flex items-center gap-3 cursor-pointer ${
                    ...
                    >
                      ...
                      <span className="text-sm font-semibold text-[#222222] flex-1">{c.text}</span>
                    </div>
                  );
                })}
              </div>
        ...
        <div className="px-6 py-4 border-t border-[#e5e5e5] flex items-center justify-between bg-[#fafafa]">
          <button
            ...
            className="btn-pill-secondary py-2 px-5 text-xs font-bold disabled:opacity-40"
          >
          ...
          <button
            ...
            onClick={() => setCurrentIndex(currentIndex + 1)}
            className="btn-pill-primary py-2 px-5 text-xs font-bold disabled:opacity-40"
          >
            <span>Next</span>
